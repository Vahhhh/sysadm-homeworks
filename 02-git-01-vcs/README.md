...
1. Создайте файлы `will_be_deleted.txt` (с текстом `will_be_deleted`) и `will_be_moved.txt` (с текстом `will_be_moved`) и закоммитте их с комментарием `Prepare to delete and move`.
1. Как вы думаете - что может случиться, если мы оставим оба файла пустыми? Почему?
